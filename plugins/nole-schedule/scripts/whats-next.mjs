 ...
 */
import { loadSchedule, resolveNow, dayStatus, meetingsOn, addDays, toMinutes, clock12, weekdayOf, SHORT, WEEK, label, staleCalendarRefusal } from './lib/schedule.mjs';
...
/* THE STALENESS GUARD, checked before the schedule is even loaded, because every
 * question this script answers -- next, today, this week -- is a question about a
 * date, and once today is past the last shipped term there is no calendar behind any
 * of them. dayStatus() already refused to INVENT a class out there, which is why
 * this was survivable; what it could not do is say why the plugin has nothing to
 * offer, and "nothing scheduled this week" reads exactly like a free week. */
const stale = staleCalendarRefusal(now, { skill: 'whats-next' });
if (stale) {
  /* next.found stays false and today's meetings stay empty in the JSON, so a
   * consumer that only reads those fields still cannot be told about a class. */
  const body = { ...stale.body, next: { found: false, reason: 'calendar-out-of-date', date: now.date }, today: { date: now.date, weekday: now.weekday, meetings: [] } };
  if (asJson) console.log(JSON.stringify(body, null, 2));
  else for (const l of stale.lines) console.log(l);
  process.exit(3);
}
