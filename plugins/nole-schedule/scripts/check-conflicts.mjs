...
import { calendarStatus, REGISTRAR_CALENDAR_URL } from './lib/campus.mjs';
...
/* THE STALENESS NOTE, AND WHY THIS SKILL DOES NOT REFUSE.
 *
 * A collision verdict is a statement about two meetings in a stored schedule, not
 * about today: "these two overlap on Tuesday at 11" stays true after the term ends.
 * So refusing here would withhold a correct answer, which is a different kind of
 * dishonesty from serving a dead deadline. What DOES go stale is everything the
 * answer points at afterwards -- drop dates, the half-term fix -- so the staleness
 * is reported as a fact on the answer rather than used to suppress it. This is the
 * one calendar-dependent skill that says it and keeps going, and that is deliberate. */
const calStale = calendarStatus(now.date);

const payload = {
  status: 'ok',
  termCode: loaded.termCode,
  sessionsStatus,
  calendarStale: calStale.stale,
  calendarSay: calStale.stale ? calStale.say : null,
...
out.push('');
if (calStale.stale) {
  out.push('THE SHIPPED CALENDAR IS OUT OF DATE');
  out.push(`  ${calStale.say}`);
  out.push('  The overlaps below are still true -- they are a fact about the stored schedule, not');
  out.push('  about today. Every DATE this answer would otherwise send you to is not: drop and');
  out.push(`  withdrawal dates for the current term are at ${REGISTRAR_CALENDAR_URL}.`);
  out.push('');
}
