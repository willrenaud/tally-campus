...
import { building, termCalendar, termCalendars, termCalendarCovering, calendarStatus } from './campus.mjs';
...
/**
 * THE STALENESS GUARD, in one place so every calendar-dependent skill refuses the
 * same way and none of them can be given a softer version of the message.
 *
 * See calendarStatus() in campus.mjs for what "stale" means and why "before the
 * term starts" is deliberately not it. This is the presentation half: a script
 * calls it immediately after resolving `now`, and if it returns non-null it prints
 * and exits 3 before reaching any code that could read a date out of the expired
 * calendar. The refusal comes FIRST, so there is no path on which a dead deadline
 * is computed and then suppressed by prose -- the same structure as the blackout
 * check in where-to-park.mjs.
 *
 * The message deliberately quotes NO date from the shipped calendar. A refusal
 * that ends "...but for reference, the drop deadline was 9 October" hands over the
 * very number it just said does not apply, and that is the number a student will
 * remember.
 */
export function staleCalendarRefusal(now, { kind = 'calendar-out-of-date', skill = null } = {}) {
  const status = calendarStatus(now.date);
  ...
  const body = {
    ...
    skill,
    date: now.date,
    calendar: { state: status.state, shippedTerms: status.shippedTerms, lastTerm: status.lastTerm },
    say: status.say,
    sendTo: status.sendTo
  };

  const lines = [
    `REFUSING TO ANSWER -- ${kind}`,
    '',
    `  ${status.say}`,
    '',
    '  This plugin ships one term at a time and this copy has not been updated. Do NOT read a',
    '  deadline, a holiday, an exam date or a next class out of it, and do not offer one "for',
    '  reference": a real date from a term that is over is the most convincing kind of wrong answer.',
    '',
    `  Send the student to the Registrar's own calendar: ${status.sendTo}`,
    `  Shipped terms in this copy: ${status.shippedTerms.join(', ') || '(none)'}.`,
    '  A maintainer fixes this by adding the new term to data/term-calendar.json -- see the',
    '  "Refreshing the calendar for a new term" section of the repository README.'
  ];

  return { body, lines, status };
}
