 ...
 */
import { parkingZones, termCalendarCovering, termCalendars, calendarStatus, REGISTRAR_CALENDAR_URL } from './campus.mjs';
...
export function blackoutCheck(date, time = '12:00') {
  ...
  if (!cal) {
    /* Two ways to have no calendar, and they need different things said about
     * them. A date BEFORE a shipped term is a gap in coverage. A date after every
     * shipped term means this copy of the plugin has simply expired -- the same
     * condition the calendar-dependent skills refuse on -- and the honest sentence
     * names that rather than implying the file merely lacks one term. Either way
     * the refusal is identical, because an unknown game status is an unknown game
     * status; only the explanation and the second link differ. */
    const cs = calendarStatus(date);
    return {
      status: 'no-calendar',
      date,
      staleCalendar: cs.stale,
      shippedTerms: termCalendars().map((t) => t.termCode),
      why: (cs.stale
        ? `THE CALENDAR DATA SHIPPED WITH THIS PLUGIN IS OUT OF DATE: the last term it covers ended ` +
          `${cs.lastTerm?.endDate}, and ${date} is past it. `
        : `No shipped term calendar covers ${date}. `) +
        'Its parkingBlackouts therefore cannot be checked, which is not the same as the date being ' +
        'clear: it means whether a home football game falls on it is unknown, and the shipped rules ' +
        'are known to be wrong on game days.',
      sendTo: 'https://transportation.fsu.edu/GameDay',
      alsoSee: cs.stale ? REGISTRAR_CALENDAR_URL : null
    };
  }
