 ...
 */
import { loadSchedule, resolveNow, addDays, weekdayOf, clock12, SHORT, staleCalendarRefusal } from './lib/schedule.mjs';
...
/* THE STALENESS GUARD, ABOVE ANY LOOKUP THAT COULD READ A DATE OUT OF THE CALENDAR.
 *
 * This is the script where an expired calendar does the most damage, and the damage
 * needs no bug to happen: termCalendar('2026-fall') keeps returning a perfectly
 * valid record forever, so an explicit --term, or a schedule imported last year and
 * never deleted, walks straight past the no-calendar refusal below and prints a drop
 * deadline belonging to a term that ended months ago. Only a comparison against
 * TODAY can catch that, and it has to happen here -- above the lookup -- so that the
 * dead dates are never computed at all rather than computed and then talked around. */
const stale = staleCalendarRefusal(now, { skill: 'deadlines' });
if (stale) {
  if (asJson) console.log(JSON.stringify(stale.body, null, 2));
