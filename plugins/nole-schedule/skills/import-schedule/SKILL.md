...
**2. An unknown building code is expected, not exceptional.** Only 33 of FSU's
...
1. **Establish the term without asking.** Run `current-term.mjs`. It returns a
   ...
   `current-term.mjs` also returns `calendarStale`. When it is true the shipped
   calendar has expired — today is past every term in the data. **Importing still
   works**; parsing, resolving buildings and storing a schedule need no calendar. But
   say so on the draft: no deadline, holiday, half-term or conflict date can be
   checked for this term, and the real calendar is at
   <https://registrar.fsu.edu/bulletins/calendar>. Do not quote a date out of the
   expired calendar to fill the gap.
...
- `schemaVersion` is the plugin version — `0.1.0`.
