...
`latitude`/`longitude`, `buildingCode`, `termCode`, `slug`, `confidence`, `sourceAttribution`.
...
- **`priority`** resolves deliberate overlap. A home-game closure sits at priority 100 over the
  everyday weekend rule, so the everyday rule never has to be edited. Highest priority wins; two
  matching rules at equal priority is a data error a consumer should surface rather than silently
  pick between.
- **No default.** When no rule matches a moment, the answer is *unknown*, not *permitted*. The data
  does not assert an open default.

Windows do not wrap past midnight — an overnight span is two rules. `capacity.typicalFullBy` is
more useful to a student than `totalSpaces`: "there are 900 spaces" is not an answer, "don't bother
after 09:30" is.
...
The examples validate cleanly under Ajv 8 in **strict mode** with `ajv-formats`, and eleven
deliberately-malformed documents are rejected — an async class with a room, an in-person class with
a null location, a biweekly lab with no anchor date, a permit-required rule with no permit list, a
building with no entrances, a swapped latitude/longitude pair, and an unknown property.
