...
| `building-not-in-data` | The building is not one of the 32 that ship — `WCB` above all | There is no coordinate for it, so no garage can be ranked against it. Naming one anyway is a guess about a building the data has never seen. |
